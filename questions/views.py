...
class QuestionListView(ListView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            user_tags = UserProfileModel.objects.get_tags(request)
            print(user_tags)
            questions = Questions.objects.filter(tags__tag_name__in=[tag for tag in user_tags])
            context['objects_list'] = Questions.objects.filter(draft=False)
            context['questions_following'] = questions_following
            context['questions'] = questions
            return context
        else:
            context['object_list'] = Questions.objects.filter(draft=False)
            return context
...
def upvote_create(request, pk, q_pk):
    answer_instance = get_object_or_404(Answers, pk=pk)
...
def question_upvote_create(request, pk):
    question_instance = get_object_or_404(Questions, pk=pk)
...
class SavedQuestionListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if request.user.is_authenticated:
            user = UserProfileModel.objects.get(user=request.user)
            questions = user.saved_questions.all()
            context["objects_list"] = questions
            return context
        else:
